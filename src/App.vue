        ...
        <form v-if="!ynab.token">
          ...
          <ul>
            <li>
              ...
              >YNAB Developer Settings</a> and create a new OAuth Application.
            </li>
            <li>
              Copy your client ID and redirect URI into
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      error: null,
      budgetId: this.$store.state.budgetId,
      budgets: [],
      transactions: this.$store.state.transactionsModule.transactions
    };
  },
  // When this component is created, check whether we need to get a token,
  // budgets or display the transactions
  created() {
    this.ynab.token = this.$store.dispatch("findYNABToken").then(token => {
      this.$store.commit("setApi", new ynab.api(token));
      if (!this.$store.getters["budgetId"]) {
        this.getBudgets();
      } else {
  ...
  methods: {
    ...
    selectBudget(id) {
      ...
      this.error = null;
      this.budgetId = id;
      this.$store.commit("setBudgetId", id);
      this.$store
        .dispatch("transactionsModule/getTransactions")
